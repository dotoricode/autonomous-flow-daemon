...
| AI가 대용량 파일 7개 읽기 (103KB) | ~25,800 토큰 소비 | **홀로그램으로 ~1,800 토큰 (93% 절감)** |
...
|:-----|:---|
| 홀로그램 요청 횟수 | 7회 |
| 대상 파일 총 크기 | ~103 KB (7개 파일, 평균 14.7 KB) |
| 원본 토큰 비용 | ~25,800 토큰 |
| 홀로그램 압축 후 | ~1,800 토큰 |
| **절약된 토큰** | **~24,000 토큰 (93% 절감)** |

### 규모별 효과

```
세션 사용 토큰:       33,000  ████████░░░░░░░░
홀로그램 절약 토큰:   24,000  ██████░░░░░░░░░░  (세션 전체의 42%)
```

세션이 길어질수록 절약 효과가 누적됩니다. ctx 50% 이상에서 대용량 파일을 홀로그램 없이 반복 읽으면 컨텍스트 윈도우가 빠르게 소진됩니다. 홀로그램을 사용하면 각 대용량 파일을 **원본의 1/14 비용**으로 읽을 수 있습니다.
