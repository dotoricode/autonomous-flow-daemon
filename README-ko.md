...
Claude Code, Cursor, Windsurf — AI 에이전트는 강력하지만 손이 거칠다. `.claudeignore` 날리고, `hooks.json` 오염시키고, `.cursorrules` 통째로 지운다. 문제를 인식할 때는 이미 늦었다. 코딩 멈추고, 원인 파악하고, 수동으로 복구. **30분 증발. 플로우 초기화.**

코드베이스를 읽을 때마다? 전체 소스 파일을 컨텍스트 윈도우에 그대로 펌핑한다. **필요도 없는 함수 본문에 수천 토큰을 태운다.**

## 해결책

`afd`는 투명한 백그라운드 데몬이다. 핵심 파일을 감시하고, 오염을 184ms 안에 치유하고, AI 에이전트에게는 원본 소스 대신 압축된 타입 스켈레톤을 제공한다. AI는 필요한 구조 정보를 1/16 비용으로 얻는다. 실수는 눈치채기 전에 수정된다. 의도적인 삭제는 존중된다. 설정 없음, 간섭 없음.
...
ctx 50%를 넘어가면 파일 읽기가 토큰 예산을 지배한다. 홀로그램 없이 대용량 파일 8개를 읽으면 매번 ~28.6K 토큰이 소비된다. 홀로그램으로는 **파일 하나당 원래 비용의 1/16** — 반복 읽기가 많을수록 격차가 벌어진다.
...
페이로드는 절대 경로, 시크릿 없이 정제되어 이식 가능하다.
